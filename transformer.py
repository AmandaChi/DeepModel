...
class WPEncoder():
    def __init__(self, d_model, d_inner_hid, n_head, d_k, d_v, layers = 1, dropout = 0.1):
        self.layers = [EncoderLayer(d_model, d_inner_hid, n_head, d_k, d_v, dropout) for _ in range(layers)]
...
class WPTransformer:
    def __init__(self, d_model, d_inner_hid, n_head, d_k, d_v, n_days, n_alpha, layers=1, dropout=0.1):
        self.encoder = WPEncoder(d_model, d_inner_hid, n_head, d_k, d_v, layers, dropout)
        self.target_layer = TimeDistributed(Dense(1, use_bias=True))
        self.n_days = n_days
        self.n_alpha = n_alphas
    def inference(self,x):
        encoder_output = self.encoder(x)
        weight_predict = self.target_layer(encoder_output)
    def compile(self, optimizer='adam',activate_layers=999):
        src_seq_input = Input(shape=(None,self.n_alphas,self.n_days),dtype='float32')
        weight_target = Input(shape=(None, self.n_alphas),dtype='float32')
        predict = self.inference(src_seq_input)
        def get_loss(args):
            pred,label = args
            return tf.losses.mean_squared_error(pred,label)
        loss = Lambda(get_loss)([predict, weight_target])
        self.ppl = Lambda(K.exp)(loss)
        self.model = Model([src_seq_input, weight_target], loss)

        self.model.add_loss([loss])
        self.output_model = Model([src_seq_input, weight_target], predict)
